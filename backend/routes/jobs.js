...
router.post("/upload", upload.single("file"), async (req, res) => {
  try {
    const {
      printer_id, pages, copies = 1, color = false,
      double_sided = false, phone = "", priority = false,
    ...
    const printerRow = await db.query("SELECT college_id FROM printers WHERE id = $1", [printer_id]);
    ...
    const result = await db.query(
      `INSERT INTO jobs
       ...
       RETURNING *`,
      [jobId, college_id, printer_id, req.file.originalname, parseInt(pages),
       parseInt(copies), isColor, isDoubleSided, cost, isPriority, pageFrom, pageTo, phone.trim() || null]
    );
  ...
  try {
    ...
    const result = await db.query(
      `SELECT j.*, p.name AS printer_name
       FROM jobs j LEFT JOIN printers p ON p.id = j.printer_id
...
router.get("/", authMiddleware, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT j.*, p.name AS printer_name
       FROM jobs j LEFT JOIN printers p ON p.id = j.printer_id
       ORDER BY j.priority DESC, j.created_at ASC LIMIT 100`
    );
    res.json({ jobs: result.rows });
  } catch (err) {
...
router.patch("/:id/status", authMiddleware, async (req, res) => {
  try {
    ...
    io.to(`job:${job.id}`).emit("job_update", { id: job.id, status: job.status, updated_at: job.updated_at });
    if (job.printer_id) {
      io.to(`printer:${job.printer_id}`).emit("queue_update", { jobId: job.id, status: job.status });
    }
